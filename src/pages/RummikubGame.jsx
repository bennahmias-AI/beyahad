...
function LocalGameScreen({ mode, difficulty, numPlayers, onBack, onExit }) {
  ...
  const handleDraw = () => {
    ...
    playSound('drop')
    setState(drawTile(state))
    if (drawn) setLastDrawn({ tile: drawn, forIdx: turnIdx })
  }
      ...
      <div style={{ padding: '12px 12px 24px' }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12, justifyContent: 'center' }}>
          {state.players.map((p, i) => (
            <OpponentChip key={p.id} player={p} active={i === turnIdx && !winner} />
          ))}
        </div>
        ...
        {/* הודעה על האריח החדש שנשלף — מופיעה כשחוזר התור */}
...
function OpponentChip({ player, active }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 18 }}>{player.isAI ? '🤖' : '👤'}</div>
      <div>
...
function PlayerRack({ rack, selectedTileId, onTileClick }) {
  return (
    <div style={{
...
function EndModal({ mode, winnerName, youWon, onPlayAgain, onExit }) {
  const emoji = youWon ? '🎉' : (mode === 'ai' ? '🤖' : '🎉')
  const title = youWon ? 'ניצחת!' : `${winnerName} ניצח!`
  const subtitle = youWon ? 'כל הכבוד — נפטרת מכל האריחים!' : 'משחק יפה — נסו שוב'
  return (
