/*
  ...
  הסנכרון: מצב המשחק המלא נשמר כ-JSON על מסמך החדר (monopolyRooms).
  בשונה מרמיקוב (תור סינכרוני), מנוע מונופול הוא אסינכרוני (הטלה →
  צעידה תא-תא → נחיתה → קלף). לכן:
    • רק מי-שבתורו מריץ את המנוע. הוא מטיל, צועד (אנימציה מקומית),
      ...
      updateMonopolyState. התור עובר לשחקן הבא, שהופך לכותב הבא.
...
import { flagSVG } from '../data/monopolyFlags.js'
import { cardBack } from '../data/monopolyBoardArt.js'
import {
...
} from '../data/monopolyBoard.js'
import {
  createMonopolyRoom, joinMonopolyRoom, startMonopolyGame,
  updateMonopolyState, watchMonopolyRoom, leaveMonopolyRoom,
  findOrCreateMonopolyMatch, watchFriendships, sendGameInvite,
  watchUser, sendMonopolyChat,
...
// ── חפיסות זמניות (זהות ל-MonopolyGame עד שבן יאשר את נתוני האקסל) ──
const LOTTO_CARDS = [
...
function initMonopolyState(playerDefs) {
