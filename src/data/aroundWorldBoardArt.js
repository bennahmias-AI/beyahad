/*
  ...
  as an HTML string + scoped CSS, rendered inside AroundWorldBoard.jsx via
  dangerouslySetInnerHTML. Visual layer only - game logic stays in
  aroundWorldBoard.js.
*/

import { TILES, GROUPS } from './aroundWorldBoard';
import { flagSVG } from './aroundWorldFlags';
...
/* ----- print-order mapping (play-order ids -> physical strip order) -----
   ...
   token/zoom geometry in AroundWorldBoard.jsx maps 1:1. Original tile rotations
...
function centerHTML(priceIndex) {
  ...
  // two face-down card decks sitting in the center, like a real classic board.
...
/* ----- scoped CSS (the original board style, prefixed .aw-stage) ----- */
...
.aw-stage *{box-sizing:border-box; margin:0; padding:0;}

.aw-stage .edge{position:relative; overflow:visible;}
.aw-stage .edge.top{grid-column:2;grid-row:1;}
.aw-stage .edge.bottom{grid-column:2;grid-row:3;}
.aw-stage .edge.left{grid-column:1;grid-row:2;}
.aw-stage .edge.right{grid-column:3;grid-row:2;}
/* divider lines between a corner and the first tile of each side strip */
.aw-stage .edge.right{border-top:2px solid var(--ink);}
.aw-stage .strip{position:absolute; display:flex; align-items:stretch; background:var(--cream);}
.aw-stage .edge.top .strip, .aw-stage .edge.bottom .strip{top:0;left:0;width:100%;height:100%;}
.aw-stage .strip.rot180{transform:rotate(180deg);}
.aw-stage .edge.left .strip, .aw-stage .edge.right .strip{
  top:50%; left:50%; width:1176px; height:212px;
}
.aw-stage .strip.rot90{transform:translate(-50%,-50%) rotate(90deg);}
.aw-stage .strip.rot-90{transform:translate(-50%,-50%) rotate(-90deg);}

.aw-stage .group{display:flex;flex-direction:column; min-width:0;}
.aw-stage .strip > *{border-inline-start:2px solid var(--ink);}
.aw-stage .strip > *:first-child{border-inline-start:none;}
.aw-stage .group-bar{
...
.aw-stage .group-cells{display:flex; flex:1; min-height:0;}
.aw-stage .slot{display:flex;}

.aw-stage .cell{flex:1; min-width:0; border-inline-start:2px solid var(--ink); position:relative; background:var(--cream);
  display:flex; flex-direction:column; align-items:stretch; text-align:center;}
.aw-stage .group .cell:first-child{border-inline-start:none;}

.aw-stage .prop .grpband{height:8px; background:var(--grp); border-bottom:1.5px solid var(--ink);}
.aw-stage .prop .flagbox{height:50px; display:flex;align-items:center;justify-content:center; padding:5px 6px 2px;}
.aw-stage .prop .flagbox .flagsvg{width:64px;height:40px; filter:drop-shadow(0 1px 0 rgba(0,0,0,.12));}
.aw-stage .prop .price{font-family:'Rubik','Heebo',sans-serif;font-weight:700;font-size:21px; line-height:1; padding:3px 0 2px; border-top:1.5px solid var(--ink); border-bottom:1.5px solid var(--ink); margin:0 0 3px;}
.aw-stage .prop .pname{font-weight:900;font-size:18px;line-height:1.02; padding:1px 3px 0; word-break:break-word;}
.aw-stage .prop .pcap{font-weight:500;font-size:12.5px; color:#3a3a3a; margin-top:1px;}
.aw-stage .prop .pinfo{margin-top:auto; padding:3px 4px 6px; display:flex;flex-direction:column;gap:1px; font-size:10.5px; color:#222;}
.aw-stage .prop .pinfo b{font-family:'Rubik','Heebo',sans-serif;font-weight:700;}

.aw-stage .special{justify-content:space-between; padding:7px 6px; background:var(--cream);}
.aw-stage .special .s-title{font-weight:800;font-size:13px;line-height:1.05;}
.aw-stage .special .s-art{flex:1; display:flex;align-items:center;justify-content:center; padding:2px;}
.aw-stage .special .s-art svg{width:84%;max-height:92px;}
.aw-stage .special .s-sub{font-weight:700;font-size:12px; color:#222;}
.aw-stage .special::after{content:"";position:absolute;inset:4px;border:2px solid var(--acc);border-radius:4px;pointer-events:none;opacity:.55;}

.aw-stage .lotto{align-items:center;justify-content:center;}
.aw-stage .lotto .badge{display:flex;flex-direction:column;align-items:center;gap:4px;}
.aw-stage .lotto .badge svg{width:64px;height:68px;}
.aw-stage .lotto .badge span{font-weight:800;font-size:12.5px;}
.aw-stage .chance{align-items:center;justify-content:center;gap:6px;}
.aw-stage .chance .qmark{position:relative;width:74px;height:58px;display:flex;align-items:center;justify-content:center;}
.aw-stage .chance .qmark svg{position:absolute;inset:0;width:100%;height:100%;}
.aw-stage .chance .qm{position:relative;font-family:'Rubik','Heebo',sans-serif;font-weight:900;font-size:28px;color:var(--ink);}
.aw-stage .chance .chance-label{font-weight:800;font-size:13px;}

.aw-stage .corner{position:relative; display:flex;flex-direction:column;align-items:center;justify-content:center;
  text-align:center; padding:12px; border:0;}
.aw-stage .corner.tl{grid-column:1;grid-row:1; background:#f3d9d2;}
.aw-stage .corner.tr{grid-column:3;grid-row:1; background:#f3d9d2;}
.aw-stage .corner.bl{grid-column:1;grid-row:3; background:#bfe0ef; padding:0;}
.aw-stage .corner.br{grid-column:3;grid-row:3; background:var(--yellow);}
.aw-stage .corner.tl{border-top:2px solid var(--ink); border-left:2px solid var(--ink);}
.aw-stage .corner.tr{border-top:2px solid var(--ink); border-right:2px solid var(--ink);}
.aw-stage .corner.bl{border-top:2px solid var(--ink); border-right:2px solid var(--ink);}
.aw-stage .corner.br{border-top:2px solid var(--ink); border-left:2px solid var(--ink);}
.aw-stage .corner .c-title{font-weight:900;font-size:20px;line-height:1.04;}
.aw-stage .corner .c-sub{font-weight:700;font-size:12.5px;margin-top:5px;max-width:170px;}
.aw-stage .corner svg{display:block;}
.aw-stage .corner.tl{transform:rotate(180deg);}
.aw-stage .corner.tr{transform:rotate(180deg);}

.aw-stage .center{grid-column:2;grid-row:2; position:relative;
...
.aw-stage .center::before{content:""; position:absolute; inset:22px; pointer-events:none; z-index:1;
  border:3px solid var(--gold); outline:8px double rgba(217,138,43,.45); outline-offset:9px;}
.aw-stage .center::after{content:""; position:absolute; inset:34px; pointer-events:none; z-index:1;
  background:
...
.aw-stage .wheel{position:absolute; transform:translate(-50%,-50%); z-index:2;
  border-radius:50%;
  background:var(--cream); border:6px solid var(--red);
  box-shadow:0 0 0 10px rgba(0,0,0,.06) inset;}
.aw-stage .wheel svg.spokes{position:absolute;inset:0;width:100%;height:100%;}
.aw-stage .globe{position:absolute;transform:translate(-50%,-50%); z-index:2;
...
.aw-stage .globe svg{position:absolute;inset:0;width:100%;height:100%;}
.aw-stage .landmark{position:absolute; display:flex;align-items:center;justify-content:center; z-index:2;}
.aw-stage .landmark svg{width:100%;height:100%;}

.aw-stage .topstrip{position:absolute; left:26px; right:26px; top:24px; height:192px; display:flex; gap:14px; align-items:stretch; z-index:3;}
.aw-stage .topstrip .titlebox{position:relative; width:212px; flex:none; padding:6px;
  display:flex;flex-direction:column;align-items:center;justify-content:center; gap:2px;
  background:var(--red); color:#fff; border:4px solid var(--ink); box-shadow:5px 5px 0 rgba(0,0,0,.18);}
.aw-stage .topstrip .titlebox .big{font-family:'Rubik','Heebo',sans-serif;font-weight:900;font-size:44px;line-height:.95;letter-spacing:1px;}
.aw-stage .topstrip .titlebox .sub{font-weight:800;font-size:22px;}
.aw-stage .panel{position:relative; flex:1; display:flex; flex-direction:column;}
.aw-stage .panel .ptitle{font-weight:700;font-size:15px;color:#fff;margin-bottom:6px;text-shadow:0 1px 0 rgba(0,0,0,.25);}
.aw-stage .pi-table{flex:1; display:grid; grid-template-columns:repeat(11,1fr) 120px;
  grid-auto-rows:1fr; border:3px solid var(--ink); background:#fff;}
.aw-stage .pi-table>div{border-inline-start:1px solid #2b4a5c; border-top:1px solid #2b4a5c;
  display:flex;align-items:center;justify-content:center; font-size:13px;}
.aw-stage .pi-table .hd{background:var(--skygrid); font-family:'Rubik','Heebo',sans-serif;font-weight:500; font-size:13px;}
.aw-stage .pi-table .reg{background:var(--green); color:#fff; font-weight:800; font-size:15px; border-inline-start:0;}
.aw-stage .pi-table .corner0{background:var(--skygrid);border-inline-start:0; font-size:12px; font-weight:700;}
.aw-stage .pi-table .cellc{background:#eef6fb;}
.aw-stage .pi-table .zero{background:#dfeef7;font-weight:700;}
.aw-stage .pi-table .cellc.mark{background:var(--red); box-shadow:0 0 0 2.5px var(--ink) inset;}

.aw-stage .c-deck{position:absolute; top:880px; width:120px; height:166px; z-index:3;}
.aw-stage .c-deck .cardback{width:120px;height:166px;
  filter:drop-shadow(3px 4px 0 rgba(0,0,0,.28));}
.aw-stage .c-deck::before{content:""; position:absolute; top:5px; left:5px;
  width:120px; height:166px; border-radius:8px; background:#0000002e; z-index:-1;}
.aw-stage .chance-deck{left:46px;}
.aw-stage .lotto-deck{right:46px;}
`;
